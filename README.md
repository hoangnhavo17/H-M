...
- **Objective:** Build a reproducible, portfolio-ready R analysis of H&M transaction behavior to inform merchandising, channel, and CRM decisions.
- **Data Used:** `articles.csv`, `customers.csv`, and `transactions_train.csv` from the Kaggle competition bundle.
- **Core Workflow:** Data preparation, category/price/channel EDA, customer spend profiling, and RFM segmentation with segment-level deep dives.
- **Main Outputs:** `README.md` (business narrative + method) and exported charts under `output/figures/` generated by `H-M_Case-Study/H-M_Case-Study.R`.
- **Key Insight Themes:** Strong category mix differences, meaningful online vs in-store patterning over time, and clear high-value cohort behavior through RFM (including best segment `545`).
